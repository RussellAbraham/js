...
function Interval(context, callback, ms, steps) {
    ...
    if (arguments.length < 2) {
        throw new TypeError("Interval - not enough arguments");
    }
    if (context) {
...
Interval.forceCall = function (object) {
    ...
    if (object.callback.call(object.context, object.INDEX, object.length, object.BACK) === false || object.isAtEnd()) {
        object.pause();
        return false;
    }
    return true;
...
Interval.prototype.start = function (reverse) {
    ...
    if (this.BACK === bBackw && (this.isAtEnd() || !this.PAUSED)) {
        return;
    }
    this.BACK = bBackw;
    this.PAUSED = false;
