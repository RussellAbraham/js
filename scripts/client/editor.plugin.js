  ...
  var methods = {
    ...
    createTableContext: function(event, cMenuUl) {
      ...
      var onSave = function() {
        ...
        $(event.target)
          .closest("table")
          .css("width", tblWidthEdt);
        if (tblHeightEdt != "")
          $(event.target)
            .closest("table")
            .css("height", tblHeightEdt);
        $(event.target)
          .closest("table")
          .attr("align", tblAlignEdt);
        $(event.target)
          .closest("table")
          .attr("border", tblBorderEdt);
        $(event.target)
          .closest("table")
          .attr("cellspacing", tblCellspacingEdt);
        $(event.target)
          .closest("table")
          .attr("cellpadding", tblCellpaddingEdt);
        $("#" + modalId).modal("hide");
        editorObj.data("editor").focus();
      };
      methods.createModal.apply(this, [
